...
ThrowPropsPlugin.to(mc, {throwProps:{x:{velocity:500, max:800, min:0}, y:{velocity:-300, max:800, min:100}, resistance:150}});
		...
		/** @private setting the unitFactor to a higher value (default is 1) reduces the chance of the auto-accelerating behavior kicking in when determining durations when the initial velocity is adequately low - imagine dragging something past a boundary and then letting go - snapping back relatively quickly should be prioritized over matching the initial velocity (at least that's the behavior most people consider intuitive). But in some situations when the units are very low (like "time" of a timeline or rotation when using radians), it can kick in too frequently so this allows tweaking. **/
		public static var defaultUnitFactors:Object = {time:1000, totalTime:1000};
...
ThrowPropsPlugin.to(mc, {throwProps:{x:{velocity:500, max:800, min:0}, y:{velocity:-300, max:700, min:100}, resistance:150}});
		...
		public static function calculateTweenDuration(target:Object, vars:Object, maxDuration:Number=10, minDuration:Number=0.2, overshootTolerance:Number=1):Number {
			var duration:Number = 0,
				...
				curProp:Object, curDuration:Number, curVelocity:Number, curResistance:Number, curVal:Number, end:Number, curClippedDuration:Number, tracker:VelocityTracker, p:String, unitFactor:Number;
				...
				if (p != "resistance" && p != "checkpoint" && p !== "preventOvershoot") {
					...
					if (typeof(curProp) === "object") {
						...
						if ("max" in curProp && end > Number(curProp.max) + _min) {
							...
							curClippedDuration = ((curVal > curProp.max && curProp.min !== curProp.max) || (curVelocity * unitFactor > -15 && curVelocity * unitFactor < 45)) ? (minDuration + (maxDuration - minDuration) * 0.1) : calculateDuration(curVal, curProp.max, curVelocity, ease, checkpoint);
							...
							curClippedDuration = ((curVal < curProp.min && curProp.min !== curProp.max) || (curVelocity * unitFactor > -45 && curVelocity * unitFactor < 15)) ? (minDuration + (maxDuration - minDuration) * 0.1) : calculateDuration(curVal, curProp.min, curVelocity, ease, checkpoint);
							if (curClippedDuration + overshootTolerance < clippedDuration) {
								clippedDuration = curClippedDuration + overshootTolerance;
							}
						}
