package 
{
	...
	public class Main extends MovieClip
	{
		...
		public var getClosestObjectDist:Number = 0;
		public var minNum:Number = 0;
		public var getMinNum:Number  = 0;
		public var closestObject:int = 0;
		...
		public function createGridTwo() 
		{
			...
			{   
				...
				if(deletedPointsArray.length > 0)
				{
					boxTwo.x = deletedPointsArray[closestObject].x;
					boxTwo.y = deletedPointsArray[closestObject].y;
					
					TextTwoText.x = deletedPointsArray[closestObject].x-7;
					TextTwoText.y = deletedPointsArray[closestObject].y-4;
				}
		...
		public function nearestObject()
		{
			...
			for (var i:uint = 0; i < deletedPointsArray.length; i++)
			{   
				xDistance = 0 - deletedPointsArray[i].x;
				yDistance = 0 - deletedPointsArray[i].y;
				getClosestObjectDist = Math.sqrt(xDistance * xDistance + yDistance * yDistance);
				
				getMinNum = Math.abs(Math.sqrt(xDistance * xDistance + yDistance * yDistance)) 
				
				if(getMinNum < minNum || i == 0) 
				{ 
					minNum = getMinNum;
					closestObject = i;
				} 
			}
			//trace(deletedPointsArray[closestObject]+"   "+closestObject+"   "+deletedPointsArray.length)
		}
		...
		public function clickOneHandler(e:MouseEvent):void
		{
			...
			nearestObject();
			createGridTwo() ;
			deletedPointsArray.splice(closestObject,1);
		...
		{  
			if (event.keyCode == 32)
			{
				//trace (deletedPointsArray.indexOf(5));
			}
		}
