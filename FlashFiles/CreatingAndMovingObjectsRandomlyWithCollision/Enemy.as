package  
{
	...
	public class Enemy extends MovieClip 
	{
		...
		public function CircleLoop():void
		{
			...
			leftBorder = 0+(this.width/2);
			topBorder = 50+(this.height/2);
			rightBorder = refM.mapp.width-(this.width/2);
			botBorder = 50+refM.mapp.height-(this.height/2);
		...
		{   
			//trace(xSpeed+"  "+ySpeed)
			//trace(this.x+"  "+this.y+"  "+botBorder+"  "+rightBorder)
			updateMovement()
		...
		{
			//this.rotation = refM.randomRange(0,360);
		...
		{	
			...
			for (var i:uint = 0; i < refM.myCircleArray.length; i++)
			{
				
				tempBall1 = refM.myCircleArray[i] as MovieClip;
				
				for (var k:uint = 0; k < refM.myCircleArray.length; k++)
				{
					tempBall2 = refM.myCircleArray[k] as MovieClip;
					
					if (tempBall1 == tempBall2) continue;
					
					if(refM.circularCollision(tempBall1,tempBall2))
					{
						//trace(tempBall2.name+"  hit: "+myCircleArray[i].name);
						//trace(tempBall1.name+"   "+tempBall2.name)
						//trace(tempBall1.xSpeed+"   "+tempBall2.xSpeed)
